...
function readProductsTemplate(data, keywords){
    ...
    var read_products_html=`
        ...
        <table class='table table-bordered table-hover'>
            ...
            <tr>
                ...
                <th class='w-25-pct'>ФИО</th>
                <th class='w-15-pct'>Паспортные данные</th>
    ...
    $.each(data.records, function(key, val) {
        ...
        read_products_html+=`<tr>
            ...
            <td>` + val.officer_id + `</td>
            <td>` + val.officer_name + `</td>
            <td>` + val.officer_pas + `</td>

            <!-- кнопки 'действий' -->
            <td>
                <!-- кнопка для просмотра товара -->
                <button class='binput bluecolor shadow btn btn-primary m-r-10px read-one-product-button' data-officer_id='` + val.officer_id  + `'>
                    Просмотреть
                </button>

                <!-- кнопка для изменения товара -->
                <button class='binput greycolor  btn-default shadow btn  m-r-10px update-product-button' data-officer_id='` + val.officer_id  + `'>
                     Редактировать
                </button>

                <!-- кнопка для удаления товара -->
                <button class='binput btn shadow btn-danger delete-product-button' data-officer_id='` + val.officer_id  + `'>
                     Удалить
                </button>
            </td>
        </tr>`;
