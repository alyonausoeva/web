...
<form id='create-product-form' action='#' method='post' border='0' class=' form_auth form light_blue'>
		...
		<div>
           <input  name='certificate_id' class='form__input input form-control ' required />
        </div>

        <div class="form__label"> Дата выдачи</div>
		<div>
        <input name='certificate_date' class='input form-control' required />
      </div>

        <div class="form__label"> id заявки </div>
		<div>
         <input name='contract_id' class='input form-control' required>
		 </div>
        
		
		 <div class="form__label"> id квитанции об оплате </div>
		 <div>
         <input name='pay_id' class='input form-control' required>
		 </div>
...
// изменяем тайтл
    ...
    $(document).on('submit', '#create-product-form', function(){
        ...
        $.ajax({
            url: "http://web.std-237.ist.mospolytech.ru/api/certificate/create.php",
            type : "POST",
