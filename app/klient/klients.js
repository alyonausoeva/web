...
function readProductsTemplate(data, keywords){
    ...
    $.each(data.records, function(key, val) {
        ...
        read_products_html+=`<tr>
            ...
            <td>` + val.klient_id + `</td>
            <td>` + val.klient_fio + `</td>
            <td>` + val.klient_pas + `</td>

            <!-- кнопки 'действий' -->
            <td>
                <!-- кнопка для просмотра товара -->
                <button class='binput bluecolor shadow btn btn-primary m-r-10px read-one-product-button' data-klient_id='` + val.klient_id  + `'>
                    Просмотреть
                </button>

                <!-- кнопка для изменения товара -->
                <button class='binput greycolor  btn-default shadow btn  m-r-10px update-product-button' data-klient_id='` + val.klient_id  + `'>
                     Редактировать
                </button>

                <!-- кнопка для удаления товара -->
                <button class='binput btn shadow btn-danger delete-product-button' data-klient_id='` + val.klient_id  + `'>
                     Удалить
                </button>
            </td>
        </tr>`;
