...
<form id='create-product-form' action='#' method='post' border='0' class=' form_auth form light_blue'>
		...
		<div>
           <input  name='department_id' class='form__input input form-control ' required />
        </div>

        <div class="form__label"> Название отдела</div>
		<div>
        <input name='department_name' class='input form-control' required />
      </div>
		 ...
		 <div>
         <input name='department_officers' class='input form-control' required>
		 </div>
...
// изменяем тайтл
    ...
    $(document).on('submit', '#create-product-form', function(){
        ...
        $.ajax({
            url: "http://web.std-237.ist.mospolytech.ru/api/department/create.php",
            type : "POST",
